...
def subsample_dataset(dataset, idxs):
    """
    #由于idxs并不是都能找到，因此这段代码中不能直接用idxs
...
#   重写
    ...
    # 更新所有属性，保持同步
    for i, (img, sample, target, uq_idx) in enumerate(zip(dataset.imgs, dataset.samples, dataset.targets, dataset.uq_idxs)):
        if i in idxs:
            new_imgs.append(img)
...
def get_equal_len_datasets(dataset1, dataset2):
    ...
    if len(dataset1) > len(dataset2):
        ...
        rand_idxs = np.random.choice(range(len(dataset2)), size=(len(dataset1)))    # dataset2的id未必是从0到长度的顺序都有的,因此这样选出的id，会比dataset1的长度小
