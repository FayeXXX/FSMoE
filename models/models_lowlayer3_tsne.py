...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if self.experts:
            ...
            self.moe = SparseMoE(ctx_dim, width, len(self.experts), top_k=cfg.top_k)
    ...
    def forward(self, im_features):
        ...
        if len(im_features) > 1:
            ...
            bias_extra, gating_output = self.moe(im_features)
        ...
        return prompts, top_feature, bias_extra.squeeze(1)
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label):   # (batch_size, channels, img_size, img_size)
        ...
        image_inter = torch.stack(image_features[:-1]).mean(dim=0)
        ...
        prompts, top_feature, bias_extra = self.prompt_learner(selected_features)  # (batch_size, n_cls, n_tkn, ctx_dim)
        ...
        return logits, image_inter, image_features[-1], bias_extra, top_feature
