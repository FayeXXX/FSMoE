...
def train(net, optimizer, trainloader, epoch, args, log):
    ...
    # for batch_idx, (data, labels, idx) in enumerate(tqdm(itertools.islice(trainloader, 0, 3))):  # for debugging
        ...
        with torch.set_grad_enabled(True):
            optimizer.zero_grad()
            loss, ss_loss, indices = net(data, labels)
            loss = loss.mean()
        ...
        for i in range(len(indices)):
            ...
            indice = indices[i].cpu().numpy().tolist()
            data_top.append((label, indice))

    df = pd.DataFrame(data_top, columns=['Label', 'Indice'])
    df.to_csv(f'{args.train_dir}/top_{epoch}.csv', index=False)
